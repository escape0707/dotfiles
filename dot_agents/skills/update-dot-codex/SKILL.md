...
4. Prefer minimalism over long explanations.
5. When a target file is outside the sandbox, request escalation instead of giving up.
6. For Codex approval rules, prefer narrow `prefix_rule` entries. Remote-mutating or externally side-effecting commands should usually be `prompt`, not `allow`.
7. Avoid recording transient branches, PR numbers, temporary paths, or conclusions that only mattered for the current task.
