...
3. **Only the Main Agent creates new rounds.**
4. **Role is contextual and relational, not fixed by model or product identity.**
5. **GitHub is the preferred control plane, not the mandatory transport for every artifact.**
6. **Manual browser transfer is first-class for sensitive, large, binary, or otherwise unsuitable data.**
7. **Published relay history moves forward; do not rewrite it by default.**
8. **User intervention inside a Subagent session is legitimate and reportable.**
9. **Nested high-capability advisory sessions are ephemeral advisory leaves, not persistent workers.**
10. **Do not invent protocol state or housekeeping files.**
...
- the user gives a repository and relay branch and asks for the pending round to be executed;
...
When delegation is chosen, the Main Agent SHOULD proactively give the user a short copy/paste bootstrap prompt. The user should not have to reconstruct repo or branch details manually.
