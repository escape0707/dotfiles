...
7. For Codex approval rules, prefer narrow `prefix_rule` entries.
   Remote-mutating or externally side-effecting commands should usually be
   `prompt`, not `allow`.
8. Avoid recording transient branches, PR numbers, temporary paths, or
   conclusions that only mattered for the current task.
